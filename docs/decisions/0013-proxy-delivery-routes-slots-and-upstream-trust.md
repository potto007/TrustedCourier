...
* The response reaches the Agent as the Upstream sent it, with `Cache-Control: no-store` in place of the Upstream's own caching headers. 3xx responses and their `Location` are returned unchanged, because the transport never follows redirects. Responses without a length and server-sent event streams are flushed as they arrive, and proxied responses are exempt from the Agent API's write timeout, since no Secret stays in locked memory while they stream.
* Requests asking for a protocol upgrade get 400. WebSockets are out of v1 (ADR-0002), and an upgraded connection would carry bytes Redaction (#6) never sees.
...
* Bad, because this narrows ADR-0012's "a Secret never becomes a Go string in the core". `net/http` takes header values only as strings, so Proxy Delivery copies the Secret into a Go string on the heap (and HTTP/2 copies it again into its header encoder). The locked copy is released as soon as the header is built; the string stays until the garbage collector reclaims its memory, which it does not wipe.
