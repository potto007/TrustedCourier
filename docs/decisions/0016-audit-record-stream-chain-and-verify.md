...
* **Written when the Delivery ends.** The record carries the Upstream's status code and, for an allowed Delivery that did not complete, a fixed `failure` text (Secret not fetched, Upstream not reached, response cut off, and so on). It never carries error text from a Backend Plugin or Upstream, which is untrusted and could echo anything. The Agent's cancellation does not stop the write. On shutdown the Agent API waits for its handlers after closing connections still open past the 5-second grace period, so a cut-off Proxy Delivery is recorded as `cut off by server shutdown` before the database closes. A failed write is logged at error level. The response has usually been sent by then, so a failed write cannot change it.
* **The hash covers the JSON encoding.** Each record's hash is the SHA-256 of its JSON object without `hash`: fixed fields in a fixed order, time as RFC 3339 in UTC at millisecond precision, `upstream_status` null when there was no response, and `prev_hash` inside it. The first record's `prev_hash` is 32 zero bytes, and `seq` counts up from 1 with no gaps. `hash` is always the last member, so a stream consumer checks a record by hashing the line as received with its final `,"hash":"…"` member removed, and never by re-encoding the JSON: encoders disagree on escaping (Go writes `<` as `<` and replaces invalid UTF-8), and an Agent chooses some of the bytes, such as a denied Secret Name. The field list and order are part of the format.
...
* **The server holds the chain head in memory.** New records chain to the last record this process appended, not to whatever is last in the database. Verify walks the last stretch of the chain holding the append lock and compares the chain's end with that head. So verify reports records removed or replaced at the end while the server runs, and the next record no longer chains. It also reports a record someone else wrote after the head, which would otherwise take the next `seq` and make every later append fail.
...
* Bad, because a Delivery whose Audit Record cannot be stored still goes ahead, logged at error level. `tc audit verify` is what surfaces the cause.
