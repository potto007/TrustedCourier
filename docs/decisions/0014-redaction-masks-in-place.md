...
* Let the transport ask for gzip and decode it, or ask Upstreams for `identity` only
...
* Every exact match of the Secret's bytes, leftmost first, becomes a run of `*` of the same length. If the Secret contains `*`, the mask is the first printable ASCII byte it does not contain, so a mask cannot join the bytes around it into a new match. `Content-Length` stays true, and a response without the Secret reaches the Agent byte for byte with every header. A fixed marker was rejected because it changes the body's length, which cannot be known before the headers go out, so every proxied response would have to drop `Content-Length`, including responses without the Secret.
...
* The Agent's `Accept-Encoding` is removed, so the transport asks the Upstream for gzip and decodes it before Redaction. A response that still has a `Content-Encoding` other than `identity`, and could have a body, gets 502, with the encoding in the server log. Asking for `identity` only was rejected because gzip costs nothing to decode here and saves the Upstream link on large LLM responses.
...
* Bad, because a Secret that contains every printable ASCII byte is masked with `*`, which it contains, so a mask and the bytes around it could form a new match.
