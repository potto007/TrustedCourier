...
**This is routing, not a protected Claude profile.** Claude Code hooks can fail open. The current broker confines jobs it receives, but no launcher confines the entire Claude Code process. A missing, disabled, malformed, or timed-out hook may let Bash run on the host; file tools, MCP servers, subagents, and computer tools do not pass through this hook. The adapter denies requested background Bash jobs; interactive PTY/stdin continuation is not implemented. Claude evaluates permissions against the rewritten dispatch command, so original-command deny and ask rules and saved dispatcher approvals need a native Claude validation before claiming normal approval behavior. Do not grant blanket dispatch permission. Do not use this development adapter as the boundary that keeps Secrets away from arbitrary agent tool code.
...
The plugin hook path resolves from `${CLAUDE_PLUGIN_ROOT}`. It submits `{ "action": "submit", "command": "...", "workdir": "..." }` over the profile's Unix socket, receives a job identifier, and rewrites Bash to `tc-exec dispatch --config ... --job ...`. The broker must validate the workspace and job ownership independently; hook output does not confer authority. In rewritten Bash calls, named requests use `/tc-exec request --resource RESOURCE --method GET --path PATH`. The worker mounts `/tc-exec` at that fixed path. A host-side manual call uses the installed binary and `--config`. Do not assume an arbitrary `curl` or `gh` call gains credentials.
