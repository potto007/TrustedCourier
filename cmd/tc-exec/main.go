...
func setup(args []string) error {
	...
	for name, contents := range map[string][]byte{"tc-exec.json": data, "hooks.json": hookData} {
...
func (b *broker) shell(j job) reply {
...
func hook(args []string) error {
	...
	replacement, e := json.Marshal(shellQuote(exe) + " dispatch --config " + shellQuote(*p) + " --job " + shellQuote(r.Job))
	if e != nil {
		return e
	}
